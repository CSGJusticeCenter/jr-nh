...
## Table 3. Percentage of individuals/bookings with Medicaid Match and Behavior Health "Encounters" by timing of BH Encounter
...
## Table 4. Percentage of high-utilizer individuals/bookings with Medicaid Match and Behavior Health "Encounters"
...
Note: Since we only have homelessness data for individuals who matched to Medicaid enrollment data, our denominator for these metrics will be the sample of individuals who have at least one Medicaid enrollment during the study window (i.e., individuals who did not match to Medicaid data are excluded here). Because of this decision as well as the fact that the DHHS data notes whether an individual was experiencing homelessness at the beginning of a given Medicaid enrollment period. In other words, we wouldn't know if someone had housing at the beginning of enrollment and then experienced homelessness afterwards). I am currently using all homelessness data from the Medicaid Enrollment files, regardless of timing (before, during, or after the study period). **Because of these factors, we should all discuss how we understand and explain this measure.** Statistics for individuals who are high utilizers of jail are based on the top 10% percentile of statewide bookings. 
...
medicaid_jail_homelessness_hu_county_table <- jail_medicaid_analytic_individual_booking_level_dedup_hu_recode %>% 
  dplyr::group_by(county) %>% 
  dplyr::summarise(`Individuals who are High Utilizers w/ Medicaid Match (N)` = n_distinct(unique_person_id[medicaid_match_flag==1 & hu_group_exclusive!="Non-HU"]),
                   `Individuals who are High Utilizers w/ any experience of homelessness (N)` = n_distinct(unique_person_id[(pre_homeless_on_eligibility_begin_flag | study_homeless_on_eligibility_begin_flag==1 | post_homeless_on_eligibility_begin_flag==1) & (hu_group_exclusive!="Non-HU")]),
                   `Individuals who are High Utilizers w/ any experience of homelessness (%)` = scales::percent(`Individuals who are High Utilizers w/ any experience of homelessness (N)`/`Individuals who are High Utilizers w/ Medicaid Match (N)`),
                   `Bookings for High Utilizers w/ Medicaid Match (N)` = n_distinct(unique_person_booking_id[medicaid_match_flag==1 & hu_group_exclusive!="Non-HU"]),
                   `Bookings for High Utilizers w/ any experience of homelessness (N)` = n_distinct(unique_person_booking_id[(pre_homeless_on_eligibility_begin_flag | study_homeless_on_eligibility_begin_flag==1 | post_homeless_on_eligibility_begin_flag==1) & (hu_group_exclusive!="Non-HU")]),
                   `Bookings for High Utilizers w/ any experience of homelessness (%)` = scales::percent(`Bookings for High Utilizers w/ any experience of homelessness (N)`/`Bookings for High Utilizers w/ Medicaid Match (N)`))  %>% 
  ungroup() %>% 
  dplyr::rename(`Jail` = county) 
...
A closer look at serious SPMI's (bipolar disorder, schizophrenia, etc.)
