...
data1 <- group_by(data1, fy) %>% mutate(pct = total/sum(total)*100) %>%
...
PRES_gg_hu_pc_holds <- ggplot(data1, aes(x = pc_hold_in_booking, y = total, fill = hu_group_exclusive )) +
  geom_col(colour = NA, position = "fill") +
  scale_y_continuous(labels = scales::percent) +
...
PRES_gg_pie_hu_pc_holds <- PRES_gg_hu_pc_holds +
...
data1 <- bookings_entrances %>%
  ...
  summarise(total = n()) %>%
  filter(pc_hold_in_booking == "PC Hold")
data1 <- group_by(data1, county) %>% mutate(pct = round(total/sum(total)*100, 0))
data1 <- as.data.frame(data1)
data1 <- data1 %>%
  mutate(hu_group_exclusive = factor(hu_group_exclusive, levels = c("Non-HU", "Top 10%","Top 5%", "Top 1%")))
...
PRES_gg_pchold_hu_prop <- ggplot(data1, aes(x = county, y = total, fill = hu_group_exclusive )) +
  ...
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(values=c("gray", jri_orange, jri_green, jri_light_blue), labels = c("Non-HU", "Top 10%","Top 5%", "Top 1%")) +
  geom_text(aes(label = ifelse(pct > 4, paste(pct, "%"), ""),
                fontface = 'bold'),
            position = position_fill(vjust = 0.5),
            vjust = 0.7,
            size = 10, family = "Franklin Gothic Book") +
...
```
**Belknap and Merrimack County had the most top 1% high utilizers who entered jail on a PC hold.**

```{r, out.width="75%", echo=FALSE, layout = "l-body"}
ggsave(PRES_gg_pchold_hu_prop, file="img/PRES_gg_pchold_hu_prop.png", width = 12,  height = 9, dpi = 100)
