...
nh_gender_male <- tidycensus::get_acs(geography = "state", 
                                      ...
                                      year = 2020) %>% 
  clean_names() %>% 
  mutate(Men = round(estimate/nh_gender_total_pop$estimate_total_pop*100, digits = 2))
...
nh_gender_female <- tidycensus::get_acs(geography = "state", 
                                      ...
                                      year = 2020) %>% 
  clean_names() %>% 
  mutate(Women = round(estimate/nh_gender_total_pop$estimate_total_pop*100, digits = 2))

### combine nh_gender_male and nh_gender_female
nh_gender_acs <- left_join(nh_gender_male,
                           nh_gender_female,
                           by = "geoid") %>% 
  dplyr::select(name=name.x,Men,Women) %>% 
  gather(gender, percentage, `Men`:`Women`) %>% 
  ### switching out bjs jail survey gender numbers for NH-specific population-wide numbers
  ### using table 3 from: https://bjs.ojp.gov/sites/g/files/xyckuh236/files/media/document/ji21st_sumB.pdf
  mutate(bjs_jail_survey_gender_percentage = ifelse(gender=="Women",
                                                    13.4,
                                                    86.6)) %>% 
  dplyr::select(-percentage) %>% 
  dplyr::rename(percentage = bjs_jail_survey_gender_percentage)
...
nh_gender_acs_to_join <- nh_gender_acs %>% 
  dplyr::select(`Gender` = gender,
                `US Jail Population Estimate (%)` = percentage)
...
df_nh_sample_state_gender_compare_bar <- df_nh_sample_state_gender_compare %>%
  dplyr::select(`Gender`,`US Jail Population Estimate (%)`,`Individuals Booked into Jail (%)`,`High Jail Utilizer, top 10% (%)`) %>%
  gather(`sample_state`, percentage, `US Jail Population Estimate (%)`:`High Jail Utilizer, top 10% (%)`) %>% 
  mutate(`sample_state` = case_when(`sample_state`=="US Jail Population Estimate (%)"~"US Jail Population",
                                    `sample_state`=="Individuals Booked into Jail (%)"~"NH Jail Population",
                                    `sample_state`=="High Jail Utilizer, top 10% (%)"~"NH High Jail Utilizer Population",
         TRUE ~ as.character(NA))) %>% 
  mutate(`sample_state` = factor(`sample_state`,
                                levels = c("US Jail Population","NH Jail Population","NH High Jail Utilizer Population"))) %>% 
  mutate(percentage=percentage/100) ### convert percentage to decimal for ggplot

### plot
ggplot(data = df_nh_sample_state_gender_compare_bar, aes(y = percentage, x = forcats::fct_rev(sample_state), fill = forcats::fct_rev(`Gender`))) + 
  geom_bar(stat = "identity", position = "dodge", color = "gray80") +
  scale_x_discrete(limits = rev) +
  scale_y_continuous(labels = scales::percent_format(accuracy=1),
                     limits=c(min(0), 
                                           max(.97))) +
  scale_fill_manual(values = rev(c("#167a9c","#293e5c"))) +
  scale_color_manual(values = rev(c("#167a9c","#293e5c"))) +
  labs(x = NULL, y = NULL,
       fill = NULL,
       caption = NULL,
       title = NULL,
...
"C:\\Users\\abyrum\\The Council of State Governments\\JC Research - Documents\\JR_NH\\Output\\r_output_presentation"

### write out
ggsave(
    filename = file.path("C:\\Users\\abyrum\\The Council of State Governments\\JC Research - Documents\\JR_NH\\Output\\r_output_presentation\\gender_overall_comparison_descriptive_women_men.png"),
    plot = last_plot(),
    device = ragg::agg_png,
    dpi = 300,
    height = 5.25,
...
### weighted estimate of percentage of race from 2020 census (18+)
...
### Race/Ethnicity, New Hampshire Adult Population (2020 Census) (excluding Grafton, Carroll, Strafford, and Hillsborough)
...
### recode 'Black Hispanic' as 'Black' for now -- the sample size is too small for analysis; discuss with Mari in light of recent research-wide conversation
### also recode 'American Indian Alaska Native' to align with census data
nh_eight_county_medicaid_match_final_race_three_counties_excluded <- jail_medicaid_analytic_individual_booking_level_dedup_hu_recode %>% 
...
nh_eight_county_medicaid_match_final_race_three_counties_excluded$id  <- nh_eight_county_medicaid_match_final_race_three_counties_excluded$unique_person_id

medicaid_jail_all_hus <- fnc_create_high_utilizer_variables(nh_eight_county_medicaid_match_final_race_three_counties_excluded)

nh_eight_county_medicaid_match_final_race <- nh_eight_county_medicaid_match_final_race_three_counties_excluded %>%
...
### all -- overall for race/ethnicity
nh_eight_county_medicaid_match_final_race_overall <- nh_eight_county_medicaid_match_final_race %>% 
  distinct(unique_person_id,
           .keep_all = TRUE) %>% 
  filter(!is.na(race)==TRUE) %>% 
  summarise(`Individuals Booked into Jail (N)` = n_distinct(unique_person_id),
...
### combine tables for kable
nh_eight_county_medicaid_match_final_race_overall_final <- rbind(nh_eight_county_medicaid_match_final_race_table,nh_eight_county_medicaid_match_final_race_overall)

### print table via kableextra
kable(nh_eight_county_medicaid_match_final_race_overall_final, 
      format.args = list(big.mark = ","), 
      align=rep('c')) %>%
   kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), row_label_position = "l") %>% 
...
### note: currently not running this analysis since we have switched to using medicaid data

### Race/Ethnicity of Jail Booking Population: PC-Hold versus non-PC Hold (**This is using jail admin data, not Medicaid data. For internal analysis only**)
...
```{r layout="l-body-outset",echo=FALSE,message=FALSE,warning=FALSE}
### prep both dataframes for ggplot
nh_eight_county_charge_clean_final_race_table_final_to_join <- nh_eight_county_medicaid_match_final_race_overall_final %>% 
...
ggplot(data = df_nh_sample_state_race_eth_compare_bar, aes(y = percentage, x = sample_state, fill = forcats::fct_rev(sample_state))) + 
  ...
  scale_y_continuous(labels = scales::percent_format(accuracy=1),
                     limits=c(min(0), 
                                           max(1))) +
  scale_fill_manual(values = c("#167a9c","#293e5c","#557e39")) +
  scale_color_manual(values = rev(c("#167a9c","#293e5c","#557e39"))) +
  labs(x = NULL, y = NULL,
       fill = NULL,
       caption = NULL,
       title = NULL,
...
```{r layout="l-body-outset",echo=FALSE,message=FALSE,warning=FALSE}
### prep census and medicaid dataframes for join and rri calculation
nh_race_2020 %<>%
  ungroup() %>% 
...
### Statewide Rates Visual: overall jail population and high utilizer population (using top 10% threshold) 
...
df_nh_race_eth_rri_analytic_table_without_am_indian <- df_nh_race_eth_rri_analytic_table %>% 
  filter(!`Race/Ethnicity`%in%c("American Indian/Alaska Native","Asian/Pacific Islander"))
...
df_nh_race_eth_rri_overall_to_plot <- df_nh_race_eth_rri_analytic_table_without_am_indian  %>% 
  ...
  mutate(rri_measure = case_when(
    ...
    rri_measure=="High Jail Utilizer, top 10% RRI (to White individuals)" ~ "High Utilizer Population",
    TRUE ~ "NA")) %>%
    mutate(rri_measure = factor(rri_measure,
                                levels = c("Overall Jail Population","High Utilizer Population"))) %>% 
...
ggplot(data = df_nh_race_eth_rri_overall_to_plot, 
  ...
  scale_y_continuous(limits=c(min(0), 
                                           max(9))) +
  scale_fill_manual(values = c("#293e5c","#167a9c")) +
  scale_color_manual(values = rev(c("#293e5c","#167a9c"))) +
  theme(legend.position = "top",
        legend.title.align=0.5,
 ...
 #                             nrow = 1)) +
  ...
  annotate("text", 
           x = 2.53, 
           y = 2.9, 
           label = "This line represents White\nindividuals (reference group)", 
           colour = "#000000", 
           size = 2.75, 
           fontface = 2)
...
### write out
ggsave(
    filename = file.path(sp_viz_output_path, "race_eth_overall_supervision_rri_option_stripe.png"),
    plot = last_plot(),
    device = ragg::agg_png,
    dpi = 300,
    height = 5.25,
