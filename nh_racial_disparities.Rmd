...
nh_eight_county_charge_clean_final_race <- nh_eight_county_charge_clean_final %>% 
  mutate(race = ifelse(race=="Black Hispanic",
                       "Black",
                       race),
         race= ifelse(race=="American Indian/Alaskan Native",
...
### write out
ggsave(
    filename = file.path(sp_viz_output_path, "race_eth_overall_comparison_descriptive.png"),
    plot = last_plot(),
    device = ragg::agg_png,
    dpi = 300,
    height = 5.25,
...
nh_eight_county_charge_clean_final_race_los <- nh_eight_county_charge_clean_final_race %>% 
  distinct(booking_id,
           .keep_all = TRUE) %>% 
  filter(!is.na(race)==TRUE) %>% 
  group_by(race) %>% 
