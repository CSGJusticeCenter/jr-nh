...
ggplot(data = df_nh_sample_state_gender_compare_bar, aes(y = percentage, x = forcats::fct_rev(sample_state), fill = `Gender`)) + 
  ...
  scale_y_continuous(labels = scales::percent_format(accuracy=1),
                     limits=c(min(0), 
                                           max(.97))) +
  scale_fill_manual(values = rev(c("#4095B1","#273C4C"))) +
  scale_color_manual(values = rev(c("#4095B1","#273C4C"))) +
  labs(x = NULL, y = NULL,
       fill = NULL,
       caption = "US jail population data were pulled from the Bureau of Justice Statistics 2021 Annual Survey of Jails (ASJ)\nNew Hampshire jail population reflects unique individuals booked into jail from 2019 through 2021",
       title = NULL,
       subtitle = NULL) +
  theme_minimal() +
...
ggplot(data = df_nh_race_eth_rri_overall_to_plot, 
  ...
  scale_y_continuous(limits=c(min(0), 
                                           max(9))) +
  scale_fill_manual(values = c("#273C4C","#4095B1")) +
  scale_color_manual(values = rev(c("#273C4C","#4095B1"))) +
  theme(legend.position = "top",
        legend.title.align=0.5,
    ...
    labs(y = "Relative Rate Index (RRI)", 
         x = NULL,
      caption = "Note: New Hampshire jail administrative data reflects unique individuals booked into jail in 2019-2021\nDue to a small sample size, individuals who are American Indian/Alaska Native or Asian/Pacific Islander are excluded from this analysis\nHillsborough and Carroll County are excluded because race/ethnicity data did not 'Hispanic' as on option\nAdditionally, Strafford County is excluded because of demographic differences\nlikely resulting from federally-contracted detention services",
      title = NULL,
      subtitle = NULL,
      fill = NULL) +
    coord_flip() +
...
### write out
ggsave(
    filename = file.path(sp_viz_output_path, "race_eth_overall_supervision_rri_option_3.png"),
    plot = last_plot(),
    device = ragg::agg_png,
    dpi = 300,
    height = 5.25,
