...
df_nh_race_eth_rri_overall_to_plot <- df_nh_race_eth_rri_analytic_table  %>% 
  ...
  mutate(rri_measure = case_when(
    rri_measure=="Individuals Booked into Jail RRI (to White individuals)" ~ "Overall Jail Population",
    rri_measure=="High Jail Utilizer, top 10% RRI (to White individuals)" ~ "High Jail Utilizer Population",
    TRUE ~ "NA")) %>%
    mutate(rri_measure = factor(rri_measure,
                                levels = c("Overall Jail Population","High Jail Utilizer Population"))) %>% 
