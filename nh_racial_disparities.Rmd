...
### set sp_viz_output_path for viz output
# sp_viz_output_path <- csg_sp_path(file.path("JR_NH/Output/r_output_presentation"))
sp_viz_output_path <- "C:/Users/mroberts/The Council of State Governments/JC Research - JR_NH/Output/r_output_presentation"
...
nh_gender_acs <- left_join(nh_gender_male,
  ...
  mutate(bjs_jail_survey_gender_percentage = ifelse(gender=="Female",
                                                    13.4,
                                                    86.6)) %>% 
...
### plot
ggplot(data = df_nh_sample_state_gender_compare_bar, aes(y = percentage, x = forcats::fct_rev(sample_state), fill = `Gender`)) + 
  geom_bar(stat = "identity", position = "dodge", color = "gray80") +
  scale_x_discrete(limits = rev) +
  scale_y_continuous(labels = scales::percent_format(accuracy=1),
                     limits=c(min(0), 
...
### write out
ggsave(
    filename = file.path(sp_viz_output_path, "gender_overall_comparison_descriptive_option_2.png"),
    plot = last_plot(),
    device = ragg::agg_png,
    dpi = 300,
    height = 5.25,
