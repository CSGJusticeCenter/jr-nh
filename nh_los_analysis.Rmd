...
los_bookings_jail_0_day <- los_bookings_jail %>% 
  filter(los_category == "0") %>% 
  filter(!is.na(sentence_status_standard))
sum <- sum(los_bookings_jail_0_day$total)
los_bookings_jail_0_day <- los_bookings_jail_0_day %>% 
  mutate(pct = total/sum*100) %>%
  mutate(pct = round(pct, 0)) %>% 
  mutate(pct = paste0(pct, "%"))

los_bookings_jail_1_day <- los_bookings_jail %>% 
  filter(los_category == "1") %>% 
  filter(!is.na(sentence_status_standard))
sum <- sum(los_bookings_jail_1_day$total)
los_bookings_jail_1_day <- los_bookings_jail_1_day %>% 
  mutate(pct = total/sum*100) %>%
  mutate(pct = round(pct, 0)) %>% 
  mutate(pct = paste0(pct, "%"))

los_bookings_jail_0_1_day <- los_bookings_jail %>% 
  filter(los_category == "1" | los_category == "0") %>% 
  filter(!is.na(sentence_status_standard)) %>%
  group_by(sentence_status_standard) %>% 
  summarise(total = sum(total))
sum <- sum(los_bookings_jail_0_1_day$total)
los_bookings_jail_0_1_day <- los_bookings_jail_0_1_day %>% 
  mutate(pct = total/sum*100) %>%
  mutate(pct = round(pct, 0)) %>% 
  mutate(pct = paste0(pct, "%")) %>% 
  mutate(state = "New Hampshire",
...
**Booking Types for People Entering Jail for 0-1 Days**

```{r}
PRES_gg_pie_0_1_los_bookings <- ggplot(los_bookings_jail_0_1_day, aes(x = state, y = total, fill = sentence_status_standard)) +
  geom_col(colour = NA, position = "fill") +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(values=c(jri_light_blue, jri_orange, jri_green, "gray")) +
  geom_text(aes(label = ifelse(total > 100, pct, ""), fontface = 'bold'), position = position_fill(vjust = 0.5),
            ...
            color = ifelse(los_bookings_jail_0_1_day$sentence_status_standard == "NA", "gray", "black")
            ) +
  theme_axes +
...
**Booking Types for People Entering Jail for 0 Days**

```{r}
PRES_gg_pie_0_los_bookings <- ggplot(los_bookings_jail_0_day, aes(x = state, y = total, fill = sentence_status_standard)) +
  geom_col(colour = NA, position = "fill") +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(values=c(jri_light_blue, jri_orange, jri_green, "gray")) +
  geom_text(aes(label = ifelse(total > 100, pct, ""), fontface = 'bold'), position = position_fill(vjust = 0.5),
            size = 7.5, family = "Franklin Gothic Book",
            color = ifelse(los_bookings_jail_0_day$sentence_status_standard == "NA", "gray", "white")
            ) +
  theme_axes +
  theme(legend.position = "top",
        legend.justification = c(0, 0),
...
**Booking Types for People Entering Jail for 1 Day**

```{r}
PRES_gg_pie_1_los_bookings <- ggplot(los_bookings_jail_1_day, aes(x = state, y = total, fill = sentence_status_standard)) +
  geom_col(colour = NA, position = "fill") +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(values=c(jri_light_blue, jri_orange, jri_green, "gray")) +
  geom_text(aes(label = ifelse(total > 100, pct, ""), fontface = 'bold'), position = position_fill(vjust = 0.5),
            size = 7.5, family = "Franklin Gothic Book",
            color = ifelse(los_bookings_jail_1_day$sentence_status_standard == "NA", "gray", "black")
            ) +
  theme_axes +
  theme(legend.position = "top",
        legend.justification = c(0, 0),
