...
PRES_gg_los_category <- data1 %>%
  ...
  theme(
    axis.text.x = element_text(size = 18, color = jri_gray, angle = 45, hjust = 0.75),
    axis.text.y = element_blank(),
    axis.title.x = element_text(size = 18, color = jri_gray),
    axis.title.y = element_text(angle = 90, size = 18, color = jri_gray),
...
PRES_gg_diverge_hu_overall_los_prop1 <- hu_overall_los_prop %>%
  ...
  labs(x = "Length of Stay (Days)\n", y = "Proportion of Entrances\n") +
   geom_text(aes(label = pct_label),
            ...
            color = ifelse(hu_overall_los_prop$hu_group_overall == "High Utilizer", jri_gray, jri_gray),
            fontface = "bold"
              ) +
  scale_y_continuous(limits = c(-.35, .35)) +
  scale_fill_manual(values=c(jri_gray, "gray")) +
  theme_no_grid_no_labels +
  coord_flip() +
...
```{r, out.width="100%", echo=FALSE, layout = "l-body"}
ggsave(PRES_gg_diverge_hu_overall_los_prop1, file="img/PRES_gg_diverge_hu_overall_los_prop1.png", width = 8,  height = 8, dpi = 100)
knitr::include_graphics("img/PRES_gg_diverge_hu_overall_los_prop1.png")
```
...
PRES_gg_diverge_hu_overall_los_prop2 <- hu_overall_los_prop %>%
  ...
  labs(x = "Length of Stay (Days)\n", y = "Proportion of Entrances\n") +
   geom_text(aes(label = pct_label),
            ...
            color = ifelse(hu_overall_los_prop$hu_group_exclusive == "Tier 1 HU", jri_light_blue, jri_gray),
            fontface = "bold"
              ) +
  scale_y_continuous(limits = c(-.35, .35)) +
...
```{r, out.width="100%", echo=FALSE, layout = "l-body"}
ggsave(PRES_gg_diverge_hu_overall_los_prop2, file="img/PRES_gg_diverge_hu_overall_los_prop2.png", width = 8,  height = 8, dpi = 100)
knitr::include_graphics("img/PRES_gg_diverge_hu_overall_los_prop2.png")
```
...
PRES_gg_pie_0_1_los_bookings <- ggplot(jail_booking_los_0_1_day, aes(x = state, y = total, fill = sentence_status_standard)) +
  ...
  theme(
    ...
    axis.text.y=element_blank(),
    legend.title = element_blank(),
    legend.text = element_text(family = "Franklin Gothic Book", size = 20, color = jri_gray))
```
...
PRES_gg_crime_los <- crime_los_summary %>%
  ggplot(aes(x = reorder(crime_type_lookup, mean), 
             y = mean, fill=factor(ifelse(crime_type_lookup=="Violent","Highlighted","Normal")))) +
  geom_bar(stat = "identity") +
  scale_fill_manual(name = "area", values=c(jri_orange,"gray")) +
      geom_text(aes(label = paste0(round(mean, 0), " Days"), fontface = 'bold'
                    ), 
                color = ifelse(crime_los_summary$crime_type_lookup=="Violent", jri_orange, jri_gray), 
  ...
  theme(legend.position = "none",
        ...
        axis.text.y = element_text(face = c("plain", "plain", "plain", "plain", "plain", "plain", "bold"), color = c(jri_gray, jri_gray, jri_gray, jri_gray, jri_gray, jri_gray, jri_orange)),
...
```
```{r, out.width="100%", echo=FALSE, layout = "l-body"}
ggsave(PRES_gg_crime_los, file="img/PRES_gg_crime_los.png", width = 10,  height = 5, dpi = 100)
knitr::include_graphics("img/PRES_gg_crime_los.png")
```
