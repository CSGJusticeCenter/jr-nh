...
# Length of Stay for Jail Entrances, Statewide from 2019 to 2021 (DHHS data)
...
**Nearly half of jail entrances (DHHS data) had lengths of stay between 0-1 days**
...
# Ggplot showing the average and median los for high utilizers
...
PRES_gg_diverge_hu_overall_los_prop <- hu_overall_los_prop %>%
  ...
  geom_text(aes(label = ifelse(pct > .02 | pct < 0, pct_label, "")),
           ...
           color = "white",
           fontface = "bold") + 
  scale_fill_manual(values=c(jri_light_blue, jri_dark_blue)) +
  theme_no_grid_no_labels +
  coord_flip() +
...
```{r, out.width="100%", echo=FALSE, layout = "l-body"}
ggsave(PRES_gg_diverge_hu_overall_los_prop, file="img/PRES_gg_diverge_hu_overall_los_prop.png", width = 9,  height = 8, dpi = 100)
...
**38 pct of jail bookings (non-PC holds) had lengths of stay between 0-1 days.**
...
PRES_gg_los_0_1_crime_type <- ggplot(los_0_1_day_crime_tier1hu_nonhu, aes(fill=group, y=pct, x=crime_type_lookup)) + 
  ...
  scale_x_discrete(breaks=unique(los_0_1_day_crime_tier1hu_nonhu$crime_type_lookup), 
    labels=addline_format(c("Drug Alcohol", 
                            "FTA Bail", 
...
For those with LOS between 0-1 days, tier 1 high utilizers were more likely to be booked for probation and parole violations compared to non-high utilizers. Not including PC holds.    
