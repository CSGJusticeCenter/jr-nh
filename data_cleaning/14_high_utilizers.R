...
df_hu_bookings_fy <-
  ...
  left_join(hu_avg_bookings_10_pct,  by = c("fy")) %>%
  left_join(hu_num_bookings_10_pct,  by = c("fy")) %>%
...
# combine tables
# round values
df_hu_bookings_fy <- rbind(df_hu_bookings_fy, df_hu_bookings_totals)
...
# get totals
df_hu_bookings_total_row <- df_hu_bookings_fy %>% filter(fy == "Total") %>% select(county = fy, everything())
...
hc_hu_bookings_month_year <-
  ...
  hc_setup() %>%
  hc_xAxis(
    #categories = list("Top 1%", "Top 5%", "Top 10%"),
...
df_hu_bookings_fy_pct <- nh_booking_no_pc_hold %>%
  select(fy, county, booking_id,
         high_utilizer_1_pct, high_utilizer_5_pct, high_utilizer_10_pct) %>%
  distinct()
df_hu_bookings_fy_pct <- gather(df_hu_bookings_fy_pct, hu, type, high_utilizer_1_pct:high_utilizer_10_pct, factor_key=TRUE)
df_hu_bookings_fy_pct <- df_hu_bookings_fy_pct %>%
  group_by(fy, hu, type) %>%
  summarise(total = n()) %>%
  mutate(hu = case_when(hu == "high_utilizer_1_pct" ~ "Top 1%",
                        hu == "high_utilizer_5_pct" ~ "Top 5%",
...
temp <- df_hu_bookings_fy_pct %>% filter(hu == "Top 1%")
gg_hu_bookings_fy_1_pct <- fnc_hu_pct_grouped_bar_chart(temp, "gray", jri_light_blue)

# top 5% proportion of bookings that are HU's by FY
temp <- df_hu_bookings_fy_pct %>% filter(hu == "Top 5%")
gg_hu_bookings_fy_5_pct <- fnc_hu_pct_grouped_bar_chart(temp, "gray", jri_green)

# top 10% proportion of bookings that are HU's by FY
temp <- df_hu_bookings_fy_pct %>% filter(hu == "Top 10%")
gg_hu_bookings_fy_10_pct <- fnc_hu_pct_grouped_bar_chart(temp, "gray", jri_orange)
...
df_hu_bookings_totals <-
  ...
  left_join(hu_avg_bookings_10_pct_3yr,  by = c("county")) %>%
  left_join(hu_num_bookings_10_pct_3yr,  by = c("county")) %>%
...
df_hu_bookings_fy_county <-
  ...
  left_join(hu_avg_bookings_10_pct,  by = c("county", "fy")) %>%
  left_join(hu_num_bookings_10_pct,  by = c("county", "fy")) %>%
...
# combine data
# round data
df_hu_bookings_fy_county <- rbind(df_hu_bookings_fy_county, df_hu_bookings_totals)
df_hu_bookings_fy_county <- df_hu_bookings_fy_county %>%
  mutate(
    avg_num_bookings_1_pct = round(avg_num_bookings_1_pct, 1),
    avg_num_bookings_5_pct = round(avg_num_bookings_5_pct, 1),
    avg_num_bookings_10_pct = round(avg_num_bookings_10_pct, 1))

df_hu_bookings_totals <- rbind(df_hu_bookings_totals, df_hu_bookings_total_row)
df_hu_bookings_totals <- df_hu_bookings_totals %>% select(county, total_bookings, everything())
...
df_hu_pc_holds_10_pct <- df_hu_pc_holds %>% filter(high_utilizer_10_pct == "Yes") %>%
  group_by(pc_hold_in_booking) %>% summarise(total = n()) %>%
  mutate(hu = "Top 10%")
df_hu_pc_holds_10_pct <- group_by(df_hu_pc_holds_10_pct) %>% mutate(pct = round(total/sum(total)*100, 1))
df_hu_pc_holds_10_pct <- as.data.frame(df_hu_pc_holds_10_pct)

df_hu_pc_holds_1510_pct <- rbind(df_hu_pc_holds_1_pct, df_hu_pc_holds_5_pct, df_hu_pc_holds_10_pct)

# gg plots of proportion of bookings that are PC holds by FY
temp <- df_hu_pc_holds_1510_pct %>% mutate(pct = comma(pct, digits = 1)) %>% mutate(pct = paste0(pct, "%"))
temp$hu <- factor(temp$hu, levels=c("Top 1%", "Top 5%", "Top 10%"))
gg_hu_pc_holds_1510_pct <- ggplot(temp, aes(x = hu, y = total, fill = pc_hold_in_booking)) +
...
##########
# reactable tables with top 1%, 5%, 10% details - data in high_utilizers.R
##########

temp <- df_hu_bookings_fy
...
# Prop bookings
save(gg_hu_bookings_fy_1_pct,         file=paste0(sp_data_path, "/Data/r_data/gg_hu_bookings_fy_1_pct.Rda",            sep = ""))
save(gg_hu_bookings_fy_5_pct,         file=paste0(sp_data_path, "/Data/r_data/gg_hu_bookings_fy_5_pct.Rda",            sep = ""))
save(gg_hu_bookings_fy_10_pct,        file=paste0(sp_data_path, "/Data/r_data/gg_hu_bookings_fy_10_pct.Rda",            sep = ""))

# PC holds
save(gg_hu_pc_holds_fy_1_pct,         file=paste0(sp_data_path, "/Data/r_data/gg_hu_pc_holds_fy_1_pct.Rda",         sep = ""))
save(gg_hu_pc_holds_fy_5_pct,         file=paste0(sp_data_path, "/Data/r_data/gg_hu_pc_holds_fy_5_pct.Rda",         sep = ""))
save(gg_hu_pc_holds_fy_10_pct,        file=paste0(sp_data_path, "/Data/r_data/gg_hu_pc_holds_fy_10_pct.Rda",         sep = ""))
save(gg_hu_pc_holds_1510_pct,         file=paste0(sp_data_path, "/Data/r_data/gg_hu_pc_holds_1510_pct.Rda",          sep = ""))
