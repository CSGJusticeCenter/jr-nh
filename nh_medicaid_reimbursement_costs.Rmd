...
Service providers were reimbursed an average of $242.16 per claim for behavioral health-related claims by people who matched to Medicaid and were incarcerated in NH jails between 2019 and 2021.  

```{r}
average_cost_per_claim <- medicaid_encounters_costs_primary_dx_2018_2021 %>% 
  dplyr::mutate(medicaid_reimb_amt = as.numeric(as.character(medicaid_reimb_amt))) %>% 
  dplyr::summarise(min    = min(medicaid_reimb_amt, na.rm = T),
