...
load(paste0(sp_data_path, "/Data/r_data/nh_hu_1_pct_prop.Rda",      sep = ""))
load(paste0(sp_data_path, "/Data/r_data/nh_hu_3_pct_prop.Rda",      sep = ""))
load(paste0(sp_data_path, "/Data/r_data/nh_hu_5_pct_prop.Rda",      sep = ""))
...
nh_booking_type, nh_sentence_status, nh_release_type
...
```{r, results='asis'}
temp <- nh_hu_1_pct_prop %>% filter(county == county_name) %>% filter(high_utilizer_1_pct == "Yes") %>% select(freq) 
temp <- as.vector(temp)
...
```{r}
df <- nh_hu_1_pct_prop %>% filter(county == county_name) %>% ungroup() %>% select(-county) %>% droplevels() 
reactable(df,
         pagination = FALSE,
         sortable = FALSE,
...
```{r, results='asis'}
temp <- nh_hu_3_pct_prop %>% filter(county == county_name) %>% filter(high_utilizer_3_pct == "Yes") %>% select(freq) 
temp <- as.vector(temp)
...
```{r}
df <- nh_hu_3_pct_prop %>% filter(county == county_name) %>% ungroup() %>% select(-county) %>% droplevels() 
reactable(df,
         pagination = FALSE,
         sortable = FALSE,
         theme = reactableTheme(cellStyle = list(display = "flex", flexDirection = "column", justifyContent = "center")),
...
```{r, results='asis'}
temp <- nh_hu_5_pct_prop %>% filter(county == county_name) %>% filter(high_utilizer_5_pct == "Yes") %>% select(freq) 
temp <- as.vector(temp)
...
```{r}
df <- nh_hu_5_pct_prop %>% filter(county == county_name) %>% ungroup() %>% select(-county) %>% droplevels() 
reactable(df,
          pagination = FALSE,
          sortable = FALSE,
          theme = reactableTheme(cellStyle = list(display = "flex", flexDirection = "column", justifyContent = "center")),
...
```{r}
df <- nh_pc_hold_recordings %>% filter(county == county_name) %>% ungroup() %>% select(-county) %>% droplevels() %>% arrange(-total)
reactable(df,
         pagination = FALSE,
         theme = reactableTheme(cellStyle = list(display = "flex", flexDirection = "column", justifyContent = "center")),
