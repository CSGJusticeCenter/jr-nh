...
entrances_dhhs <- medicaid_jail_all %>% 
  mutate(id = unique_person_id,
         ...
         jail_los = as.numeric(difftime(release_date,
                                        booking_date,
                                        units="days"))) %>% 
  # to ensure each individual/booking is in correct grouping, start with Non-HU, then Tier 3 and move down to Tier 1
...
data1 <- hu_table %>%
  ...
  distinct() %>%
  # mutate(hu_group_exclusive = factor(hu_group_exclusive, levels = c("Non-HU", "Tier 3","Tier 2", "Tier 1")))
...
```{r, out.width="50%", echo=FALSE, layout = "l-body"}
ggsave(PRES_gg_hu_pct_barchart, file="img/PRES_gg_hu_pct_barchart.png", width = 6,  height = 5, dpi = 100, bg='transparent')
