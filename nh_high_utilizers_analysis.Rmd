...
We defined people who are high utilizers of the jail system as the tier 1, tier 2, or tier 3 of people entering jails from FY2019 to FY2021. De-identified jail administrative data was provided by DHHS, which includes a unique id for each person to account for entrances at more than one jail.   
...
```{r}
# Add one day to those with zero days so that they are counted
...
reactable(daily_encounters,
          ...
          columns = list(
            ...
            state = colDef(show = F),
            pct_label = colDef(show = F),
            encounter_days     = colDef(show = T, minWidth = 100, name = "Number of Bed Days"),
            pct                = colDef(show = T, minWidth = 130, name = "Proportion of Bed Days", format = colFormat(percent = TRUE, digits = 1))
          )
)
```
...
```{r, out.width="50%", echo=FALSE, layout = "l-body"}
ggsave(PRES_gg_pie_daily_encounters, file="img/PRES_gg_pie_daily_encounters.png", width = 5,  height = 5, dpi = 100, bg='transparent')
knitr::include_graphics("img/PRES_gg_pie_daily_encounters.png")
```
...
PRES_gg_hu_pct_piechart <- ggplot(data1, aes(x = county, y = total_hu_entrances, fill = hu_group_exclusive)) +
  geom_col(colour = NA, position = "fill") +
  scale_y_continuous(labels = scales::percent) +
...
```{r, out.width="50%", echo=FALSE, layout = "l-body"}
ggsave(PRES_gg_hu_pct_piechart, file="img/PRES_gg_hu_pct_piechart.png", width = 6,  height = 5, dpi = 100, bg='transparent')
knitr::include_graphics("img/PRES_gg_hu_pct_piechart.png")
```
...
```{r}
# Get number of people by HU group and proportion out of all people entering NJ jails
...
# One stacked barplot for all three years then create a pie chart
PRES_gg_hu_people_pct_piechart <- ggplot(data1, aes(x = county, y = total_hu_people, fill = hu_group_exclusive)) +
  geom_col(colour = NA, position = "fill") +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(values=c(jri_light_blue, jri_green, jri_orange, "gray"), labels = c("Tier 1 HU","Tier 2 HU", "Tier 3 HU", "Non-HU")) +
  # geom_text(aes(label = pct, fontface = 'bold'), position = position_fill(vjust = 0.5),
...
```{r, out.width="50%", echo=FALSE, layout = "l-body"}
ggsave(PRES_gg_hu_people_pct_piechart, file="img/PRES_gg_hu_people_pct_piechart.png", width = 6,  height = 5, dpi = 100, bg='transparent')
knitr::include_graphics("img/PRES_gg_hu_people_pct_piechart.png")
```
