...
# subset data and save for through out this R file
df_hu_all <- bookings_entrances %>%
  select(fy,
         county,
         id,
         booking_id,
...
df_hu_graphs <- df_hu_all %>%
  group_by(fy, high_utilizer_4_times) %>%
  summarise(total = n())
df_hu_graphs <- group_by(df_hu_graphs, fy) %>%
  mutate(pct = round(total/sum(total)*100, 1))
df_hu_graphs <- as.data.frame(df_hu_graphs)
df_hu_graphs <- df_hu_graphs %>% mutate(pct = round(pct, 1))

gg_hu_4_times_fy <- ggplot(df_hu_graphs, aes(x = fy, y = total, fill = high_utilizer_4_times)) +
  geom_col(colour = NA, position = "fill") +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(values=c("gray", jri_orange), labels = c("Non-HU      ","HU")) +
  geom_text(data=subset(data1, pct > 3), aes(label = paste0(pct, "%"), fontface = 'bold'), position = position_fill(vjust = 0.5),
            size = 7.5, family = "Franklin Gothic Book",
            color = ifelse(data1$high_utilizer_4_times == "No", "black", "white")) +
  theme_axes +
...
gg_hu_entrance_prop_amount_fy <- ggplot(df_hu_graphs, aes(x = fy, y = total, fill = high_utilizer_4_times)) +
...
df_hu_entrances_fy <- df_hu_graphs %>% filter(high_utilizer_4_times == "Yes")
...
data1 <- df_hu_all %>% filter(high_utilizer_4_times == "Yes")
temp <- fnc_covid_time_highchart(data1, "", "", jri_orange)
