
# Data Availability
load(paste0(sp_data_path, "/Data/r_data/data_dictionaries_page/raw_adm_data_availability_table.Rda", sep = ""))

# Data dictionaries
load(paste0(sp_data_path, "/Data/r_data/adm_all.Rda",               sep = ""))
load(paste0(sp_data_path, "/Data/r_data/bookings_entrances.Rda",    sep = ""))

load(paste0(sp_data_path, "/Data/r_data/belknap_adm.Rda",      sep = ""))
load(paste0(sp_data_path, "/Data/r_data/carroll_adm.Rda",      sep = ""))
load(paste0(sp_data_path, "/Data/r_data/cheshire_adm.Rda",     sep = ""))
load(paste0(sp_data_path, "/Data/r_data/coos_adm.Rda",         sep = ""))
load(paste0(sp_data_path, "/Data/r_data/hillsborough_adm.Rda", sep = ""))
load(paste0(sp_data_path, "/Data/r_data/merrimack_adm.Rda",    sep = ""))
load(paste0(sp_data_path, "/Data/r_data/rockingham_adm.Rda",   sep = ""))
load(paste0(sp_data_path, "/Data/r_data/strafford_adm.Rda",    sep = ""))
load(paste0(sp_data_path, "/Data/r_data/sullivan_adm.Rda",     sep = ""))

# County Pages
load(paste0(sp_data_path, "/Data/r_data/county_pages/county_hu_1_pct_prop.Rda",  sep = ""))
load(paste0(sp_data_path, "/Data/r_data/county_pages/county_hu_5_pct_prop.Rda",  sep = ""))
load(paste0(sp_data_path, "/Data/r_data/county_pages/county_hu_10_pct_prop.Rda", sep = ""))
load(paste0(sp_data_path, "/Data/r_data/county_pages/county_race.Rda",           sep = ""))
load(paste0(sp_data_path, "/Data/r_data/county_pages/county_age_category.Rda",   sep = ""))
load(paste0(sp_data_path, "/Data/r_data/county_pages/county_gender.Rda",         sep = ""))

# Incarceration Patterns Page
load(paste0(sp_data_path, "/Data/r_data/incarceration_patterns_page/amt_people_booked.Rda",        sep = ""))
load(paste0(sp_data_path, "/Data/r_data/incarceration_patterns_page/row_people_booked.Rda",        sep = ""))
load(paste0(sp_data_path, "/Data/r_data/incarceration_patterns_page/amt_bookings.Rda",             sep = ""))
load(paste0(sp_data_path, "/Data/r_data/incarceration_patterns_page/row_bookings_fy.Rda",          sep = ""))
load(paste0(sp_data_path, "/Data/r_data/incarceration_patterns_page/PRES_gg_bookings.Rda",         sep = ""))
load(paste0(sp_data_path, "/Data/r_data/incarceration_patterns_page/table_bookings_fy_county.Rda", sep = ""))

load(paste0(sp_data_path, "/Data/r_data/incarceration_patterns_page/amt_people_entered.Rda",             sep = ""))
load(paste0(sp_data_path, "/Data/r_data/incarceration_patterns_page/row_people_entered.Rda",             sep = ""))
load(paste0(sp_data_path, "/Data/r_data/incarceration_patterns_page/amt_entrances.Rda",                  sep = ""))
load(paste0(sp_data_path, "/Data/r_data/incarceration_patterns_page/row_entrances_fy.Rda",               sep = ""))
load(paste0(sp_data_path, "/Data/r_data/incarceration_patterns_page/PRES_gg_entrances.Rda",              sep = ""))
load(paste0(sp_data_path, "/Data/r_data/incarceration_patterns_page/PRES_table_entrances_fy_county.Rda", sep = ""))
load(paste0(sp_data_path, "/Data/r_data/incarceration_patterns_page/PRES_table_entrances_people_county.Rda", sep = ""))
load(paste0(sp_data_path, "/Data/r_data/incarceration_patterns_page/PRES_gg_num_entrances.Rda", sep = ""))

# PC Holds Page
load(paste0(sp_data_path, "/Data/r_data/pc_holds_page/amt_pch_pct.Rda",                        sep = ""))
load(paste0(sp_data_path, "/Data/r_data/pc_holds_page/PRES_gg_pch_pct_barchart.Rda",           sep = ""))
load(paste0(sp_data_path, "/Data/r_data/pc_holds_page/hc_pch_time.Rda",                        sep = ""))
load(paste0(sp_data_path, "/Data/r_data/pc_holds_page/PRES_table_entrances_with_pc_holds.Rda", sep = ""))
load(paste0(sp_data_path, "/Data/r_data/pc_holds_page/table_pc_holds_fy_county.Rda",           sep = ""))

# High Utilizers Page
load(paste0(sp_data_path, "/Data/r_data/high_utilizers_page/table_hu_4_times_summary.Rda",           sep = ""))
load(paste0(sp_data_path, "/Data/r_data/high_utilizers_page/table_hu_1_pct_summary.Rda",             sep = ""))
load(paste0(sp_data_path, "/Data/r_data/high_utilizers_page/table_hu_5_pct_summary.Rda",             sep = ""))
load(paste0(sp_data_path, "/Data/r_data/high_utilizers_page/table_hu_10_pct_summary.Rda",            sep = ""))
load(paste0(sp_data_path, "/Data/r_data/high_utilizers_page/PRES_hu_summary.Rda",                    sep = ""))
load(paste0(sp_data_path, "/Data/r_data/high_utilizers_page/PRES_hu_summary1.Rda",                   sep = ""))
