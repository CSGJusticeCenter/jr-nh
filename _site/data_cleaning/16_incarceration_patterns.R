...
length(unique(nh_booking$id))         # 32188
length(unique(nh_booking$booking_id)) # 51580

# save booking dates
all_booking_dates <- nh_booking %>% select(county, id, booking_id, booking_date, month_year_text, month_year, fy) %>% distinct()
dim(all_booking_dates); length(unique(all_booking_dates$booking_id)) # 51580
...
df_bookings_events_all <- nh_booking %>%
  ...
  distinct()
dim(df_bookings_events_all); length(unique(df_bookings_events_all$booking_id))           # 51580

# smaller df without pc hold variable
df_bookings_events_distinct <- df_bookings_events_all %>%
  distinct()
dim(df_bookings_events_distinct); length(unique(df_bookings_events_distinct$booking_id)) # 51580
...
save(nh_pc_hold_recordings,         file=paste0(sp_data_path, "/Data/r_data/nh_pc_hold_recordings.Rda",         sep = ""))
