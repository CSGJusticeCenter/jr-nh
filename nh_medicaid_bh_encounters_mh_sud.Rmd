...
unique_ids_hu <- jail_medicaid_analytic_individual_booking_level_dedup_hu_recode %>% 
  ...
  distinct() %>% 
  mutate_at(c("pre_or_study_window_medicaid_match_flag_overall"), ~replace_na(.,0))
...
```{r}
# get number of people for denominator
...
smi_primary_diagnoses_hu <- medicaid_encounters_cleaned %>% 
  ...
  ungroup() %>% 
  mutate(pct = (total_people/sum(total_people))*100,
         pct = round(pct, 0),
...
```
```{r, out.width="100%", echo=FALSE, layout = "l-body"}
ggsave(PRES_gg_diverge_hu_nonhu_smi, file="img/PRES_gg_diverge_hu_nonhu_smi.png", width = 15,  height = 6, dpi = 100)
