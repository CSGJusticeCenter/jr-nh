...
### From Uma: were claims filed during incarceration?
```{r echo=FALSE,message=FALSE,warning=FALSE} 
...
uma_claims_question_df <- inner_join(medicaid_jail_all,
                                    medicaid_encounters_clean,
                                    by = "unique_person_id") %>% 
  mutate(claim_during_incarceration_flag = ifelse(first_dos_dt>booking_date &
                                                           first_dos_dt<release_date,
                                                   1,0))

# statewide table
uma_claims_question_df_table <- uma_claims_question_df %>% 
  dplyr::summarise(`Overall Individuals (N)` = n_distinct(unique_person_id),
                   `Individuals w/ Claims During Incarceration (N)` = n_distinct(unique_person_id[claim_during_incarceration_flag==1]),
                   `Individuals w/ Claims During Incarceration (%)` = n_distinct(unique_person_id[claim_during_incarceration_flag==1])/`Overall Individuals (N)`*100) 

### print table via kableextra
kable(uma_claims_question_df_table, 
      format.args = list(big.mark = ","), 
      align=rep('c')) %>%
  kable_styling(bootstrap_options = c("striped", 
                                       "hover", 
...
# statewide table
uma_claims_question_df_table_average <- uma_claims_question_df %>% 
  group_by(unique_person_id) %>% 
  dplyr::mutate(claim_count = sum(claim_during_incarceration_flag)) %>% 
  ungroup() %>% 
  summarise(`Mean Claims` = mean(claim_count,na.rm=TRUE),
            `Median Claims` = median(claim_count,na.rm=TRUE))

### print table via kableextra
kable(uma_claims_question_df_table_average, 
      format.args = list(big.mark = ","), 
      align=rep('c')) %>%
  kable_styling(bootstrap_options = c("striped", 
                                       "hover", 
