...
### Table 5. Percentage of People with BH needs who were Experiencing Homelessness at time of Medicaid Eligibility 

```{r}
# Create homeless variable anf filter to Medicaid matches
entrances_dhhs <- jail_medicaid_analytic_individual_booking_level_dedup_hu_recode %>% 
  filter(medicaid_match_flag == 1) %>% 
  mutate_at(c("overall_bh_flag",
              "pre_homeless_on_eligibility_begin_flag",
              "study_homeless_on_eligibility_begin_flag",
              "post_homeless_on_eligibility_begin_flag"), ~replace_na(.,0)) %>% 
  mutate(homeless = case_when(pre_homeless_on_eligibility_begin_flag   == 1 | 
                              study_homeless_on_eligibility_begin_flag == 1 | 
                              post_homeless_on_eligibility_begin_flag  == 1   ~ "Homeless",
                              pre_homeless_on_eligibility_begin_flag   == 0 & 
                              study_homeless_on_eligibility_begin_flag == 0 & 
                              post_homeless_on_eligibility_begin_flag  == 0   ~ "Not Homeless")) 
```

```{r}
# Total number of people by county (Medicaid = 1/bh flag = 1)
bh_people_county <- jail_medicaid_analytic_individual_booking_level_dedup_hu_recode %>%   
  filter(medicaid_match_flag == 1 & overall_bh_flag == 1) %>% 
  dplyr::ungroup() %>%
  dplyr::select(unique_person_id, county) %>%
  dplyr::distinct() %>%
  dplyr::group_by(county) %>%
  dplyr::summarise(bh_number_of_people = n()) 

# Count number and proportion of people who were homeless (Medicaid = 1/bh flag = 1) by county
bh_homeless_county <- entrances_dhhs %>% 
  filter(medicaid_match_flag == 1 & overall_bh_flag == 1) %>% 
  select(unique_person_id, county, homeless) %>% 
  distinct() %>% 
  group_by(county, homeless) %>% 
  summarise(total = n()) %>% 
  spread(homeless, total) %>% 
  clean_names() %>% 
  rename(bh_homeless = homeless,
         bh_not_homeless = not_homeless) %>% 
  left_join(bh_people_county, by = "county") %>% 
  mutate(bh_homeless_pct    = bh_homeless/bh_number_of_people) %>% 
  select(county, bh_number_of_people, everything()) %>% 
  select(-bh_not_homeless)
```

```{r}
# Total number of people by county (Medicaid = 1/bh flag = 0)
no_bh_people_county <- jail_medicaid_analytic_individual_booking_level_dedup_hu_recode %>%   
  filter(medicaid_match_flag == 1 & overall_bh_flag == 0) %>% 
  dplyr::ungroup() %>%
  dplyr::select(unique_person_id, county) %>%
  dplyr::distinct() %>%
  dplyr::group_by(county) %>%
  dplyr::summarise(no_bh_number_of_people = n()) 

# Count number and proportion of people who were homeless (Medicaid = 1/bh flag = 0) by county
no_bh_homeless_county <- entrances_dhhs %>% 
  filter(medicaid_match_flag == 1 & overall_bh_flag == 0) %>% 
  select(unique_person_id, county, homeless) %>% 
  distinct() %>% 
  group_by(county, homeless) %>% 
  ...
  spread(homeless, total) %>% 
  clean_names() %>% 
  rename(no_bh_homeless = homeless,
         no_bh_not_homeless = not_homeless) %>% 
  left_join(no_bh_people_county, by = "county") %>% 
  mutate(no_bh_homeless_pct    = no_bh_homeless/no_bh_number_of_people) %>% 
  select(county, no_bh_number_of_people, everything()) %>% 
  select(-no_bh_not_homeless)

# Combine data
bh_homeless_overview <- bh_homeless_county %>% 
  left_join(no_bh_homeless_county, by = "county") 
```

```{r}
# # Reactable table
# reactable(bh_homeless_overview,
...
#           fullWidth = FALSE,
#           columnGroups = list(
#             colGroup(name = "People with BH Encounters", columns = c("bh_number_of_people", "bh_homeless", "bh_homeless_pct")),
#             colGroup(name = "People without BH Encounters", columns = c("no_bh_number_of_people", "no_bh_homeless", "no_bh_homeless_pct"))),
#           columns = list(
#             county                 = colDef(name = "County", minWidth = 200, style = list(fontWeight = "bold")),
#             bh_number_of_people    = colDef(name = "People Matched to Medicaid and with BH Encounters (N)"),
#             bh_homeless            = colDef(name = "People Experiencing Homelessness (N)"),
#             bh_homeless_pct        = colDef(name = "People Experiencing Homelessness (%)", format = colFormat(percent = TRUE, digits = 0),
#                                             style = list(position = "sticky", borderRight = "1px solid #d3d3d3")),
#             no_bh_number_of_people = colDef(name = "People Matched to Medicaid and without BH Encounters (N)"),
#             no_bh_homeless         = colDef(name = "People Experiencing Homelessness (N)"),
#             no_bh_homeless_pct     = colDef(name = "People Experiencing Homelessness (%)", format = colFormat(percent = TRUE, digits = 0))
#             )
#           ) %>%
