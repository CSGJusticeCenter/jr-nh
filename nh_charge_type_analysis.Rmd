...
ggplot(data = df_nh_charge_type_overall, aes(y = `Bookings, Percent of Sample (%)`, x = reorder(`Charge Type`,-`Unique Bookings (N)`), fill = forcats::fct_rev(`Charge Type`))) + 
  ...
  scale_fill_manual(values = c("#167a9c","#167a9c","#167a9c","#167a9c","#167a9c","#167a9c","#167a9c","#167a9c","#167a9c")) +
  guides(fill = FALSE) +
  labs(x = NULL, y = NULL,
       fill = NULL,
       caption = NULL,
...
ggplot(data = df_county_level_charge_type_summary_long, 
  ...
  scale_fill_manual(values = c("#167a9c",
                               "#293e5c",
                               "#b95826",
                               "#87C2D4",
                               "#557e39")) +
...
### Figure 7a (take two). Charge Type by High Utilizer Grouping (Non-PC Holds Only), Bar Plot (based on all three years of data) 

This visual focuses specifically on VOP


```{r layout="l-body-outset",echo=FALSE,message=FALSE,warning=FALSE}
### to recode high utlizer percentile grouping, we don't need to group_by as the percentile grouping is already grouped across individual from Mari's cleaning
nh_eight_county_charge_clean_final_crime_type_recode_non_pc_hu_vop <- nh_eight_county_charge_clean_final_crime_type_recode_non_pc %>% 
  # filter(!crime_type_clean%in%c("Other",
  #                               "FTA/Bail",
  #                               "Temporary Hold")) %>% 
  ### to ensure each individual/booking is in correct grouping, start with Non-HU, then 10% and move down to 1%
...
### unique bookings per charge type by HU grouping
df_hu_charge_type_summary_vop <- nh_eight_county_charge_clean_final_crime_type_recode_non_pc_hu_vop %>% 
  dplyr::group_by(hu_group_exclusive) %>% 
  dplyr::summarise(`Unique Non-PC Bookings (N)` = n_distinct(unique_booking_id_nh),
                   `Probation/Parole Violation` = n_distinct(unique_booking_id_nh[crime_type_clean=="Probation/Parole Violation"])/`Unique Non-PC Bookings (N)`,
                    `Other` = n_distinct(unique_booking_id_nh[crime_type_clean!="Probation/Parole Violation"])/`Unique Non-PC Bookings (N)`) %>% 
  ungroup() %>% 
  distinct() %>% 
  dplyr::rename(`High Utilizer Percentiles` = hu_group_exclusive) 

### pivot df long for ggplot 
df_hu_charge_type_summary_long_vop <- df_hu_charge_type_summary_vop %>%
  gather(charge_type, percent, `Probation/Parole Violation`:`Other`) %>% 
  mutate(percent_label = scales::percent(percent, 
                                         accuracy = 1)) %>% 
  mutate(charge_type_factor =  factor(charge_type,
                                      levels = c("Probation/Parole Violation","Other")))

### ggplot
ggplot(data = df_hu_charge_type_summary_long_vop, 
       aes(y = percent, 
           x = `High Utilizer Percentiles`, 
           fill = forcats::fct_rev(charge_type_factor))) + 
  geom_bar(stat = "identity", 
  ...
  scale_x_discrete(limits = rev) +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(values = rev(c("#167a9c",
                               "gray"))) +
  guides(fill = guide_legend(reverse = TRUE,
                             title.position = "top",
                             label.position = "bottom",
  ...
  geom_label(aes(y = percent,
    ...
    position = position_stack(vjust = .5),
    size = 5,
    colour = case_when(df_hu_charge_type_summary_long_vop$charge_type_factor == "Other" ~ "black",
                              TRUE ~ "white"),
    show.legend = FALSE) +
  labs(x = NULL, y = NULL,
       fill = "Charge Type",
...
### write out
ggsave(
    filename = file.path(sp_viz_output_path, "charge_type_by_hu_percentiles_statewide_vop_highlight.png"),
    plot = last_plot(),
    device = ragg::agg_png,
    dpi = 300,
    height = 5.25,
...
### Figure 7a (take three). Charge Type by High Utilizer Grouping (Non-PC Holds Only), Bar Plot (based on all three years of data) 
...
### pivot df long for ggplot 
df_hu_charge_type_summary_long_vop <- df_hu_charge_type_summary_vop %>%
  gather(charge_type, percent, `Probation/Parole Violation`) %>% 
  mutate(percent_label = scales::percent(percent, 
                                         accuracy = 1))

### bar plot
ggplot(data = df_hu_charge_type_summary_long_vop, aes(y = percent, x = `High Utilizer Percentiles`, fill = forcats::fct_rev(`High Utilizer Percentiles`))) + 
  geom_bar(stat = "identity",  color = "gray80") +
  scale_x_discrete(limits = rev) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1),
                     limits = c(min(0), max(.4))) +
  ...
  scale_fill_manual(values = c("#2F7084","#2F7084","#2F7084","#2F7084","#2F7084")) +
  guides(fill = FALSE) +
  labs(x = NULL, y = NULL,
       fill = NULL,
       caption = NULL,
...
### write out
ggsave(
    filename = file.path(sp_viz_output_path, "charge_type_by_hu_percentiles_statewide_vop_highlight_option_2.png"),
    plot = last_plot(),
    device = ragg::agg_png,
    dpi = 300,
    height = 5.25,
...
### Figure 7a (take four). Charge Type by High Utilizer Grouping (Non-PC Holds Only), Bar Plot (based on all three years of data) 

This visual focuses specifically on violent offenses
...
df_hu_charge_type_summary_vop <- nh_eight_county_charge_clean_final_crime_type_recode_non_pc_hu_vop %>% 
  ...
  dplyr::summarise(`Unique Non-PC Bookings (N)` = n_distinct(unique_booking_id_nh),
                  `Violent` = n_distinct(unique_booking_id_nh[crime_type_clean=="Violent"])/`Unique Non-PC Bookings (N)`,
                    `Other` = n_distinct(unique_booking_id_nh[crime_type_clean!="Violent"])/`Unique Non-PC Bookings (N)`) %>% 
  ungroup() %>% 
  distinct() %>% 
  dplyr::rename(`High Utilizer Percentiles` = hu_group_exclusive) 
...
### pivot df long for ggplot 
df_hu_charge_type_summary_long_vop <- df_hu_charge_type_summary_vop %>%
  gather(charge_type, percent, `Violent`:`Other`) %>% 
  mutate(percent_label = scales::percent(percent, 
                                         accuracy = 1)) %>% 
  mutate(charge_type_factor =  factor(charge_type,
                                      levels = c("Violent","Other")))
...
### write out
ggsave(
    filename = file.path(sp_viz_output_path, "charge_type_by_hu_percentiles_statewide_violent_highlight.png"),
    plot = last_plot(),
    device = ragg::agg_png,
    dpi = 300,
    height = 5.25,
...
### Figure 7a (take five). Charge Type by High Utilizer Grouping (Non-PC Holds Only), Bar Plot (based on all three years of data) 
...
### pivot df long for ggplot 
df_hu_charge_type_summary_long_vop <- df_hu_charge_type_summary_vop %>%
  gather(charge_type, percent, `Violent`) %>% 
  mutate(percent_label = scales::percent(percent, 
                                         accuracy = 1))
...
### write out
ggsave(
    filename = file.path(sp_viz_output_path, "charge_type_by_hu_percentiles_statewide_violent_highlight_option_2.png"),
    plot = last_plot(),
    device = ragg::agg_png,
    dpi = 300,
    height = 5.25,
