...
Note: These booking frequencies by charge type are grouped by fiscal year of booking and exclude both probation/parole charges as well as unclassified charges (i.e., "other")

```{r layout="l-body-outset",echo=FALSE,message=FALSE,warning=FALSE}
### unique bookings per charge type by fiscal year
df_fy_charge_type_summary <- nh_eight_county_charge_clean_final_crime_type_recode_non_pc %>% 
  filter(crime_type_clean!="Other",
         crime_type_clean!="Probation/Parole Violation") %>% 
  dplyr::group_by(fy) %>% 
  dplyr::summarise(`Unique Non-PC Bookings (N)` = n_distinct(unique_booking_id_nh),
...
df_fy_charge_type_summary_long <- df_fy_charge_type_summary %>%
  gather(charge_type, percent, `Violent`:`Public Order`) %>% 
...
### Charge Type by Jail (Non-PC Holds Only), Bar Plot

Note: These booking frequencies by charge type are grouped by county of booking and exclude both probation/parole charges as well as unclassified charges (i.e., "other")
...
df_county_level_charge_type_summary_geo <- merge(nh_county_shape, 
...
### merge new district shapefile to nh data to attach geography and render the file map-able
df_county_level_public_order_summary_geo <- merge(nh_county_shape, 
...
### make labels for map's legend
label_drug_alcohol_percent <- c("7% and Greater","5% to 7%","4% to 5%","4% and Lower")
