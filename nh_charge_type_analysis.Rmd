...
# ### call additional geo-specific libraries needed for mapping
# library(easypackages)
# libraries("sf","acs","tigris","mapview","rgeos","raster")
  ...
  dplyr::summarise(`Unique Non-PC Bookings (N)` = n_distinct(unique_booking_id_nh),
                   ...
                   `Bookings for Public Order Charge (%)` = scales::percent(n_distinct(unique_booking_id_nh[crime_type_clean=="Public Order"])/`Unique Non-PC Bookings (N)`,
                    ...
                    `Bookings for Probation/Parole Violation (%)` = scales::percent(n_distinct(unique_booking_id_nh[crime_type_clean=="Probation/Parole Violation"])/`Unique Non-PC Bookings (N)`,
                                                                       accuracy = .1),
                   `Bookings for Other FTA/Bail (%)` = scales::percent(n_distinct(unique_booking_id_nh[crime_type_clean=="FTA/Bail"])/`Unique Non-PC Bookings (N)`,
                                                                       accuracy = .1),
                   `Bookings for Other Temporary Hold (%)` = scales::percent(n_distinct(unique_booking_id_nh[crime_type_clean=="Temporary Hold"])/`Unique Non-PC Bookings (N)`,
                                                                       accuracy = .1),
...
Note: I am considering a booking to be 'unique' if the following values are together unique: id, inmate_id, booking_id, county (including county as we can't verify that each county is using different IDs for individuals and bookings). The count reported in the following tables is the unique number of bookings for which the most serious charge type is a violent charge, property charge, etc. If the charge(s) for a given booking do not fall into one of the charge type category, the charge is represented as "Other" in these visualizations.

<br>

### Charge Type across State (Non-PC Holds Only)
...
### Charge Type by County (Non-PC Holds Only), Bar Plot
...
### % Drug/Alcohol Charges by Jail (Non-PC Holds Only), Map

Note: This analysis of the % of bookings which are drug/alcohol charges are presented by county of booking and include both probation/parole charges as well as unclassified charges (i.e., "other") unlike the prior two visuals. This map highlights county-level variation but grouping counties into four equally sized groups or quartiles.
...
df_county_level_charge_type_summary_geo <- left_join(nh_county_shape, 
                                                 df_county_level_charge_type_summary, 
                                                 by = "county")
...
label_drug_alcohol_percent <- c("17% and Greater","12% to 17%","9% to 12%","9% and Lower")
...
df_county_level_charge_type_summary_geo_to_map <- df_county_level_charge_type_summary_geo %>%
  mutate(percent_drug_alcohol_quartile = cut(-`Drug/Alcohol`,
                               breaks = -quantiles,
                               right = TRUE,
...
ggplot(
 ...
 scale_fill_manual(
    ...
    name = "% Bookings for Drug/Alcohol\n Charges",
    na.value = "grey50",
    guide = guide_legend(
...
### % Public Order Charges by Jail (Non-PC Holds Only), Map

Note: This analysis of the % of bookings which are public order charges are presented by county of booking and include both probation/parole charges as well as unclassified charges (i.e., "other") unlike the prior two visuals. This map highlights county-level variation but grouping counties into four equally sized groups or quartiles.
...
### Charge Type by High Utilizer Grouping (Non-PC Holds Only), Bar Plot (based on all three years of data) 
...
### % Probation/Parole Violation Charges by Jail for High Utlizers (based on all three years of data), Map

Note: This analysis of the % of bookings which are probation/parole violations for the top 10% of HU are presented by county of booking. This map highlights county-level variation but grouping counties into four equally sized groups or quartiles. Analysis excludes FTA/Bail, temporary hold, and unclassified charges (i.e., "other").
...
### unique bookings per charge type by HU grouping
df_hu_charge_type_probation_parole_summary <- nh_eight_county_charge_clean_final_crime_type_recode_non_pc_hu_recode_pb_pa_violation_map %>% 
  filter(!crime_type_clean%in%c("Other",
                                "FTA/Bail",
                                "Temporary Hold")) %>% 
...
### merge new district shapefile to nh data to attach geography and render the file map-able
df_hu_charge_type_probation_parole_summary_geo <- left_join(nh_county_shape, 
                                                 df_hu_charge_type_probation_parole_summary, 
                                                 by = "county")
...
### extract quantiles from var we're mapping
quantiles <- df_hu_charge_type_probation_parole_summary_geo %>%
  pull(`Probation/Parole Violation`) %>%
  quantile(probs = seq(0, 1, length.out = no_classes + 1), 
           na.rm=TRUE) %>%
  as.vector() 

### make labels for map's legend
label_pb_pa_violation_percent <- c("21% and Greater","17% to 21%","15% to 17%","15% and Lower")

### create a new variable with the specified quantiles
df_hu_charge_type_probation_parole_summary_geo_to_map <- df_hu_charge_type_probation_parole_summary_geo %>%
  mutate(percent_pb_pa_violation_quartile = cut(-`Probation/Parole Violation`,
                               breaks = -quantiles,
                               right = TRUE,
                               labels = label_pb_pa_violation_percent,
                               include.lowest = T)) 
...
ggplot(
  # define main data source
  data = df_hu_charge_type_probation_parole_summary_geo_to_map
) +
  scale_alpha(name = "",
              range = c(0.6, 0),
              guide = F) + # suppress legend
  ...
  geom_sf(
    mapping = aes(
      fill = percent_pb_pa_violation_quartile
      ),
    color = "white",
    size = 0.1
  ) +
 scale_fill_manual(
    values = c("#1F4A58","#2F7084","#87C2D4","#AFD6E2"),
    name = "% Bookings for Probation/Parole\n Violations\n(Top 10% of High Utilizers)",
    na.value = "grey50",
    guide = guide_legend(
     keyheight = unit(5, units = "mm"),
     title.position = "top",
