...
df <- belknap_adm %>% dplyr::select_if(not_all_na)
view_df(df)
```
...
```{r}
df <- carroll_adm %>% dplyr::select_if(not_all_na)
view_df(df)
```
...
```{r}
df <- cheshire_adm %>% dplyr::select_if(not_all_na)
view_df(df)
```
...
```{r}
df <- coos_adm %>% dplyr::select_if(not_all_na)
view_df(df)
```
...
```{r warning=FALSE}
df <-hillsborough_adm %>% dplyr::select_if(not_all_na)
view_df(df)
```
...
```{r}
df <- merrimack_adm %>% dplyr::select_if(not_all_na)
view_df(df)
```
...
```{r}
df <- rockingham_adm %>% dplyr::select_if(not_all_na)
view_df(df)
```
...
```{r}
df <- strafford_adm %>% dplyr::select_if(not_all_na)
view_df(df)
```
...
```{r}
df <- sullivan_adm %>% dplyr::select_if(not_all_na)
view_df(df)
```
