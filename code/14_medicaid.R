...
#########################################################################################
### 3/29 change (AB): de-duping in a different way to see if numbers change substantially
### our approach to de-duping only impacts analysis involving counts of claims/encounters
### or median/means of counts 
#########################################################################################
...
medicaid_enrollment_categories_encounters_2018_2021_individual_level <- medicaid_enrollment_categories_encounters %>%
  ...
  mutate(
    # study_window_medicaid_match_flag = ifelse(eligibility_end_date >= as_date("2018-07-01") & eligibility_begin_date <= as_date("2021-06-30"),
    #                                                    1,0),
  ...
  mutate(
    # study_window_medicaid_match_flag_overall = max(study_window_medicaid_match_flag,
    #                                                         na.rm=TRUE),
