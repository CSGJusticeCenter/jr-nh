...
# - De-dup by individual/booking -- keeping most serious crime type? (hierarchy: violent, property, drug/alcohol, public order, vop, fta/bail, temp. hold, other)
...
### I think the best option, at this point, is to just categorize the charges into violent, public order, drugs/alcohol, and property
  ...
  dplyr::select(charge_code_lookup, descriptor_lookup, statute_title_lookup, crime_type_lookup=crime_type)
...
# custom function to clean charge data
belknap_adm_charge_clean_final <- fnc_clean_charge_data(belknap_adm_charge_clean)
...
### 305/2680 (i.e. what % of non-pc holds have missing charge data) = 11.3%
...
# custom function to clean charge data
carroll_adm_charge_clean_final <- fnc_clean_charge_data(carroll_adm_charge_clean)
...
# custom function to clean charge data
cheshire_adm_charge_clean_final <- fnc_clean_charge_data(cheshire_adm_charge_clean)
...
### 3/2981 (i.e. what % of non-pc holds have missing charge data) = 0.01%
...
# custom function to clean charge data
coos_adm_charge_clean_final <- fnc_clean_charge_data(coos_adm_charge_clean)
...
# custom function to clean charge data
hillsborough_adm_charge_clean_final <- fnc_clean_charge_data(hillsborough_adm_charge_clean)

### remove hillsborough-specific columns for eventual rbind with other jails
hillsborough_adm_charge_clean_final <- hillsborough_adm_charge_clean_final %>%
  dplyr::select(-c(charge_desc_first_clean_count,
                   ...
                   charge_desc_second_clean_value_1_num_only))

### excluding PC holds, see what % of non-pc hold bookings join to lookup table

### let's see how many non-pc holdings are still missing charge data after cleaning and joining to the lookup table
### missing clean charge data for 101 non-PC bookings -- all of which are missing charge codes and descriptions in jail-provided data
### 101/10239 (i.e. what % of non-pc holds have missing charge data) = 1%
...
# custom function to clean charge data
merrimack_adm_charge_clean_final <- fnc_clean_charge_data(merrimack_adm_charge_clean)
...
### 125/4504 (i.e. what % of non-pc holds have missing charge data) = 2.7%
...
# custom function to clean charge data
rockingham_adm_charge_clean_final <- fnc_clean_charge_data(rockingham_adm_charge_clean)
...
### 16/6076 (i.e. what % of non-pc holds have missing charge data) = 0.3%
...
# custom function to clean charge data
sullivan_adm_charge_clean_final <- fnc_clean_charge_data(sullivan_adm_charge_clean)
...
### 26/1441 (i.e. what % of non-pc holds have missing charge data) = 1.8%
...
save(belknap_adm_charge_clean_final, file=paste0(sp_data_path, "/Data/analysis/r_data/offenses_clean/county/belknap_offenses_clean.Rda", sep = ""))
save(carroll_adm_charge_clean_final, file=paste0(sp_data_path, "/Data/analysis/r_data/offenses_clean/county/carroll_offenses_clean.Rda", sep = ""))
save(cheshire_adm_charge_clean_final, file=paste0(sp_data_path, "/Data/analysis/r_data/offenses_clean/county/cheshire_offenses_clean.Rda", sep = ""))
save(coos_adm_charge_clean_final, file=paste0(sp_data_path, "/Data/analysis/r_data/offenses_clean/county/coos_offenses_clean.Rda", sep = ""))
save(hillsborough_adm_charge_clean_final, file=paste0(sp_data_path, "/Data/analysis/r_data/offenses_clean/county/hillsborough_offenses_clean.Rda", sep = ""))
save(merrimack_adm_charge_clean_final, file=paste0(sp_data_path, "/Data/analysis/r_data/offenses_clean/county/merrimack_offenses_clean.Rda", sep = ""))
save(rockingham_adm_charge_clean_final, file=paste0(sp_data_path, "/Data/analysis/r_data/offenses_clean/county/rockingham_offenses_clean.Rda", sep = ""))
save(sullivan_adm_charge_clean_final, file=paste0(sp_data_path, "/Data/analysis/r_data/offenses_clean/county/sullivan_offenses_clean.Rda", sep = ""))

### overall file
save(nh_eight_county_charge_clean_final, file=paste0(sp_data_path, "/Data/analysis/r_data/offenses_clean/nh_eight_county_charge_clean_final.Rda", sep = ""))
