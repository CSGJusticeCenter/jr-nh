...
data1 <- data1 %>% select(-c(sum, difference))
data1 <- rbind(data1, temp)

data1 <- group_by(data1, sentence_status_standard) %>% mutate(pct = total/sum(total)*100) %>%
...
data1$sentence_status_standard <- factor(data1$sentence_status_standard,levels = c("Other",
                                                                               "Sentenced",
                                                                               "Protective Custody",
                                                                               "Pretrial"))

# percent barchart of booking types
ggplot(data1, aes(x = sentence_status_standard, y = total, fill = yesno)) +
  ...
  geom_text(aes(label = pct, fontface = 'bold'), position = position_fill(vjust = 0.5),
            ...
            color = ifelse(data1$yesno == "No", "lightgray", "white")) +
  theme_no_axes +
...
data1 <- gather(table_pct, booking_type, percent, `Other (%)`:`Unknown (%)`, factor_key=TRUE)
data1 <- data1 %>% filter(booking_type != "Unknown (%)" & booking_type != "Protective Custody (%)") %>% 
