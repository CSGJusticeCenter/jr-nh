...
```{r}
data1 <- bookings_entrances %>% select(sentence_status_standard, booking_id) %>% distinct() %>% 
...
PRES_gg_pie_pretrial <- ggplot(data1, aes(x = state, y = total, fill = sentence_status_standard)) +
  ...
  geom_text(aes(label = pct, fontface = 'bold'), position = position_fill(vjust = 0.5),
            ...
            color = ifelse(data1$sentence_status_standard == "Other", "gray", "white")) +
  theme_axes +
...
PRES_gg_pie_pretrial <- PRES_gg_pie_pretrial +
...
### High Utilizers and Booking Types (Row sums)
...
### High Utilizers and Booking Types (Column sums)
...
PRES_gg_drug_court_bookings <- ggplot(data1, aes(x = drug_court, y = total, fill = hu_group_exclusive )) +
  geom_col(colour = NA, position = "fill") +
  scale_y_continuous(labels = scales::percent) +
...
# Pie chart for all three years
PRES_gg_pie_drug_court_bookings <- PRES_gg_drug_court_bookings +
  coord_polar("y", start=0) +
  theme_no_axes +
  theme_minimal()+
  theme(
