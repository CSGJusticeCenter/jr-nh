...
The following booking patterns were found using jail administrative data submitted directly to CSG.DHHS matched data does not have information on booking types.

## Figure 1. Booking Types State-wide
...
## Table 4. Charge Type by Booking Type (Row Sums)
...
table1 <- table1 %>% 
       rename("Charge Type"            = 1,
              "NH State Prisoner (N)"  = 2, 
              "Other (N)"              = 3,
              "Pretrial (N)"           = 4,
              "Protective Custody (N)" = 5,
              "Sentenced (N)"          = 6,
              "Unknown (N)"            = 7,
              "NH State Prisoner (%)"  = 8, 
              "Other (%)"              = 9,
              ...
              "Sentenced (%)"          = 12,
              "Unknown (%)"            = 13) 
table_n   <- table1 %>% select(1:7)
table_pct <- table1 %>% select(1, 8:13)
